...
### 1. Nix Evaluation Query (NixOS / Home Manager option)
Every configuration generated by the framework automatically exports its metadata under `config.flake-modules`. You can query it using `nix eval`:

```bash
# Query NixOS host metadata
nix eval .#nixosConfigurations.testhost.config.flake-modules

# Query Standalone Home Manager user metadata
nix eval .#homeConfigurations."testuser@testhost".config.flake-modules
```

This returns an attribute set containing:
- `hostname`: Explicitly configured hostname or `"unspecified"`.
- `system`: Target system architecture.
- `users` (or `user` for HM): Active users.
- `modules`: List of all transitively resolved modules (names and source paths).
- `secrets`: Attribute set of all declared secrets, indicating their scope, required status, and description.

### 2. Programmatic Library Query
You can call the exported `queryHost` library function directly in your Nix code:

```nix
flake-modules.lib.queryHost {
  hostPath = ./hosts/testhost;
  inherit inputs;
}
```
